...
fun main() {
    ...
    val cheatMap = mutableMapOf<Pair<Int, Int>, Cheat>()
...
fun collectCheats(
    ...
    cheatLevel: Int,
    cheatDone: Boolean,
    cheatMap: MutableMap<Pair<Int, Int>, Cheat>
) {
    ...
    if (!isInside(nextPos, maze, cheatLevel == 1)) return
    ...
    val nextCheatDone = cheatDone || cheatLevel == MAX_CHEAT || (inCheat && cellValue!='#')
    ...
    if (nextPos == endPos) {
        ...
        val currentCheat = cheatMap.getOrPut(cheat.steps.first) { cheat }
        if (currentCheat.savings > cheat.savings)
            cheatMap[currentCheat.steps.first] = cheat
        return
    }
    ...
    var nextCheatLevel = if(nextCheatDone) 0  else cheatLevel+1
...
fun buildCheat(visited: MutableList<Step>, costSoFar: Int): Cheat? {
    ...
    val second = visited.findLast { it.dir == '*' }?.pos
